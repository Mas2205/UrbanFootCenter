...
const BookingPageContent = () => {
  ...
  const calculateTotalPrice = (): number => {
    ...
    let total = (field.pricePerHour || 0) * formik.values.duration;
    if (formik.values.equipmentRental && field.equipmentFee !== undefined) {
      total += (field.equipmentFee || 0) * formik.values.numberOfPeople;
    }
    return total;
  };
                  ...
                  label={
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Typography sx={{ mr: 1 }}>
                        ...
                        {field ? ` (${field.equipmentFee || 0} FCFA ${t('booking.perPerson')})` : ''}
                      </Typography>
                ...
                <Box sx={{ mb: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                    <Typography variant="body2">
                      {t('booking.fieldRental')}: {field?.pricePerHour?.toLocaleString() || '0'} FCFA x {formik.values.duration} {formik.values.duration === 1 ? t('booking.hour') : t('booking.hours')}
                    </Typography>
                    <Typography variant="body2">
                      {((field?.pricePerHour || 0) * formik.values.duration).toLocaleString()} FCFA
                    </Typography>
                  </Box>
                  ...
                  {formik.values.equipmentRental && (
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                      ...
                      <Typography variant="body2">
                        {((field?.equipmentFee || 0) * formik.values.numberOfPeople).toLocaleString()} FCFA
                      </Typography>
                    </Box>
                  )}
                </Box>
